...
export function getChordFunction(chord: Chord, key: Note): ChordFunction {
  const isMajor = key.isMajor();
  const scale = new Scale(key.toString(), isMajor ? 'major' : 'minor');
...
export function getFunctionalHarmony(scale: Scale): FunctionalHarmony {
  ...
  return {
    tonic: new Chord(notes[0], isMajor ? '' : 'm'),
    supertonic: new Chord(notes[1], 'm'),
    mediant: new Chord(notes[2], isMajor ? 'm' : ''),
    subdominant: new Chord(notes[3], isMajor ? '' : 'm'),
    dominant: new Chord(notes[4], ''),
    submediant: new Chord(notes[5], isMajor ? 'm' : ''),
    leadingTone: new Chord(notes[6], 'dim')
  };
}
