...
from .models import GameState, GameConfig, GameAction
...
class GameUI:
    ...
    def _tick(self) -> None:
        ...
        if self.on_tick and callable(self.on_tick):
            self.on_tick(state)  # type: ignore[misc]

        if state.game_over and self.on_game_over and callable(self.on_game_over):
            self.on_game_over(state)  # type: ignore[misc]
            self.stop()
